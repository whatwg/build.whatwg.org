...
app.use(route.post("/wattsi", function* () {
  ...
  const linkfixupjsFilePath = getFilePath(parts, "linkfixupjs", this);

  const outDirectory = randomDirectoryName();
  yield mkdirp(outDirectory);

  const args = [sourceFilePath, outDirectory, caniuseFilePath, w3cbugsFilePath, linkfixupjsFilePath];
